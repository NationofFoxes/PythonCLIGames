from utils_aws import GameProps
    ...
    match gp.task:
        ...
        case "connect":
            ...
            match gp.state["category"]:
                ...
                case "move":
                    ...
                    # await if offline
                    update_displays_await_move(gp)  # next is player 1's move
                    return response
        ...
        case "move":
            if gp.is_valid_move():
                ...
                save_to_db(gp)  # save to db before letting players make moves
                # await if offline
                update_displays_await_move(gp)  # next player's move
                return response
            else:
                # await if offline
...
# async if offline
def update_displays_await_move(gp):
    # send updated display to player who made move
    # await if offline
    gp.ws.send({"task": "update_display_wait", "display": gp.display})
    # send updated display to next player
    # await if offline
    gp.ws.send({"task": "update_display_your_turn", "display": gp.display}, connection_id=gp.next_connection_id)
    return
